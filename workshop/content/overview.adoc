...
In the hands on lab you will understand the nuances of how to setup an application to continue working during a disaster and recovered on another site/cluster. We will use a real life e-retailer application for a fictitious company called Globex selling goods online. Learn how to Deploy and setup this application for business continuity. 
Further, we will test the data persistency across failover/failback scenarios by updating the data in the database. Verify that the data is consistent and available after transition from one site/cluster to another.
...
The lab is divided into 5 main sections (expecting 15 minutes per section, last section may take more or less time depending on how comfortable are you with the previous section understanding):

* Understand the Implementation spects of the environment.
