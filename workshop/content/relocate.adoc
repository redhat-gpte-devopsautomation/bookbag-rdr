...
[arabic]
. You will now attempt to relocate/failback the application to the primary cluster.  Before we do that, we want to ensure that any changes made on the secondary cluster are preserved when relocating back to the primary cluster. Open a terminal, login to the secondary cluster %api_url% with your credentials username: {hub_openshift_cluster_admin_username} and password: {hub_openshift_cluster_admin_password}.
+
[source, bash]
----
oc login -u {hub_openshift_cluster_admin_username} -p {hub_openshift_cluster_admin_password} https://api.cluster-x9sc5-2.sandbox2623.opentlc.com
----
+
