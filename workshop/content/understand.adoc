...
Navigate to `ALL CLUSTERS`, `InfraSturcture`, `Clusters`, Select `Cluster Sets` tab and then select `clusterset1` and `Submariner add-ons` tab. A successful deployment will show `Connection status` and `Agent status` as `Healthy` for both `primary` and `secondary`.
